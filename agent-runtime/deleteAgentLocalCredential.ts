 ...
 * - Public catalog projections (`agent-pub-*` / `cybot-pub-*`) must not delete the
...
export function isPublicAgentProjectionKey(dbKey: string): boolean {
  const key = typeof dbKey === "string" ? dbKey.trim() : "";
  return key.startsWith("agent-pub-") || key.startsWith("cybot-pub-");
}
...
export async function deleteAgentLocalCredentialRef(
  ...
  const ref =
    typeof credentialRef === "string" ? credentialRef.trim() : "";
  if (!ref) {
