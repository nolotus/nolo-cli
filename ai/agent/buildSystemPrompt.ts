...
2. 派发原则：子任务自包含、边界清晰；**给执行者写清任务、验收和禁区，保证任务确定性**（结果可核对、不传无关历史、明确完成条件），派发后轻轮询结果，不假设成功。
3. 独立审查（commit 前硬门）：除 ≤2 步零逻辑风险的机械改动外，所有代码变更 commit 前必须先派**其他 agent**（不同模型家族优先）review。reviewer 不可是自己。无 review 不 commit——这是硬门，不是建议。多个独立改动面优先并行派多个 reviewer 各审各的。详细流程见 coding-review（代码层）的 Dispatch 规范。
