...
/** 根据屏幕宽度生成响应式布局建议（TUI & Web 优化版） */
const buildResponseGuidelines = (isMobile: boolean): string => {
  if (isMobile) {
    return `--- 响应展示指南（窄屏 / TUI / 移动端） ---
当前处于窄屏或终端视图，请采用紧凑、高扫描率的排版：
- 【结构节奏】结论或关键状态置顶，拒绝密集长段落；每段保持 2-3 行，善用「---」或引用块做卡片化切分。
- 【符号层级】用明确视觉锚点（如 ▸、•、1.、✔ / ✘ / ⚠）替代平铺叙述，缩进不超过 2 层。
- 【防折行破碎】严禁使用 ≥4 列的大宽表（终端下折行会严重错位），改用「键值清单」或纵向卡片；文件路径、命令及标识符一律用反引号 \` \` 包裹保护。
- 【紧凑代码】代码块尽量只保留关键上下文与必要行，避免横向超长行。`;
  }
  return `--- 响应展示指南（宽屏 / 桌面端） ---
当前处于大屏视图，可充分发挥空间优势：
- 【结构化全景】提供层次清晰的逻辑树、流程图或结构化推导，重要模块使用二级/三级标题清晰划分。
- 【对比与表格】适合使用 Markdown 多列表格进行方案横向对比、参数汇总或矩阵分析。
- 【代码完整度】可展示更完整的文件上下文、类型定义与多步执行示例，配合清晰的注释说明。`;
};
