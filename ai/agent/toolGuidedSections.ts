...
本系统支持多 agent 和多 review，但编排不是目的：**简单任务自己完成；复杂任务先自行分析，按领域/独立验收标准拆分并派发**。默认不为“凑数量”派发，但对跨领域、多文件、长链路任务，默认先建立并行 wave；只有能说明“主 Agent 自做更便宜且同样可靠”时才省略派发。每轮只推进当前最有价值的动作，读取结果和证据后再判断下一步。

**长期派发规则（适用于本次及以后所有任务）**：
- 先按独立领域拆分，而不是按文件数量拆分：例如架构/后端、前端、测试、文档/迁移、发布/安全各自独立时，优先并行派发。
- 只读审计、方案比较、边界扫描、测试设计等相互独立的工作，优先并行派发；父 Agent 汇总后再决定实现路径。
- 具有共享接口或强顺序依赖的工作，先由父 Agent 固化契约，再派发后续实现；不要让多个 Agent 猜同一接口。
- 一个任务同时跨越 3 个以上领域，或包含仓库改动、验证、发布/迁移等长链路时，默认至少派发 2 个独立子任务/审计；若不派发，必须在计划中记录成本/收益理由。
- 子 Agent 只负责自包含的领域交付，父 Agent 负责目标、契约、集成、最终测试和用户沟通；不要把整个模糊目标原样转发给子 Agent。
- 派发不是“把所有活甩出去”：主 Agent 可以保留最了解上下文的集成/决策工作，但不能因为连续操作方便就包办所有独立领域。
