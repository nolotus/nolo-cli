...
用 startAgentRun 启动子 Agent（异步 fork+exec 返回 runId，或 wait:true 同步拿结果），用 controlAgentRun 观察/停止（wait+signal+proc 语义）。何时该派发见「多 Agent 协作」段；本段只讲派发出去之后怎么选人、怎么盯、怎么排错。
...
1. 选人：只认 listAgents 返回的记录。
   ...
   - **tools 字段只反映额外能力，不反映 coding 能力**：writeFile/editFile/execBash/applyEdit/gitCommit 等是桌面端和 CLI 运行时的默认基线，派发时由 host 自动注入。tools=[] 不代表不能写代码，不要据此排除候选。
   - 顶档模型（Opus 5、GPT-5.6 Sol 及同级）的自动委托硬门：仅用于复杂架构/跨域设计、重大事故、安全/数据完整性高风险分析、达标的深 review，或低价候选已有失败证据后的升级。深 review 达标线＝改动文件数 ≥ 30 且触及计费/安全/数据完整性/核心路由，或低价 reviewer 已 BLOCK/通道失败。普通 review 默认派中档低价模型（DeepSeek V4 Flash、agy-flash、GLM 等）。选顶档要在回复里说明理由；用户点名不受此限。
   - 不要凭名字编造能力，不要索取 prompt/密钥/数据库 key 来选人。派发前跳过已知坏通道（配置缺失/区域限制/网关 400）。

2. 盯梢：**异步派发后立即收尾，等终态通知。**
   - 要立即拿结果就别用异步：<100s 的子任务直接 startAgentRun({ wait: true })；已经异步派出去的用 controlAgentRun(action:"wait", runId) 阻塞到终态。
   - 支持终态唤醒的环境（桌面 TUI 本地运行时）里，run 到达终态会自动提交摘要唤醒你。所以派发后若没有不依赖该结果的并行工作，一句话收尾结束回合即可。没有终态唤醒的环境（裸 CLI、服务端 runtime）用 wait 阻塞，同样不要自己循环查。
   ...
   - controlAgentRun 只在「答案会改变你下一步动作」时用：能否开始汇总、要不要叫停、要不要补派。一次性检查用 status(runId, tailLines:0)，返回的 progress（工具调用数、inFlight、idleMs）足够判断死活，不拉日志省 token。
...
3. 排错：先分诊，再下结论。
   ...
   - 判定「派发通道整体不可用」前，至少对 2 个不同候选各完成「已验证 key + 一次真实派发」。候选不足 2 个就如实报告「仅此候选且通道失败」，不得夸大成全库不可用，也不得擅自改做本该派发的事。不要拿「不同 provider 家族」当硬门槛，有多家才换。
...
3. 派发质量：子任务必须自包含、边界清晰，写清目标、已确认事实、约束、相关证据、验收标准和禁区。
   - **上下文最小化**：只传完成该子任务所需的最小工作集。不要为「保险」转发无关工具、旧日志、重复文件内容或整段历史。自己读完工具结果后先提炼关键事实，后续用提炼后的版本，需要原文再定向读取。
