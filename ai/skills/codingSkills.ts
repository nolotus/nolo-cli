...
/**
 * Ensure coding skill pages exist for the given userId.
 * Returns a thunk suitable for dispatch. Best-effort: failures don't block.
 */
export function ensureCodingSkills(
  userId: string,
): (dispatch: any) => Promise<void> {
  return async (dispatch: any) => {
    const { readAndWait, write } = await import("../../database/dbSlice");
    const { DataType } = await import("../../create/types");
    ...
    for (const page of pages) {
      try {
        const existing = await dispatch(readAndWait(page.dbKey))
          .unwrap()
          .catch(() => null);
        if (!existing) {
          await dispatch(
