...
import { fetchUserSpaceMemberships } from "../create/space/spaceSlice";
...
export interface AuthState {
  ...
  isLoading: boolean;
  /**
   * initializeAuth 是否已完成（无论成功/失败）。用于区分"初始化进行中"
   * 与"已加载且确实未登录"——AdminRoute 等权限 gate 需等它为 true 才判权限，
   * 否则已登录管理员刷新页面时 initializeAuth 还没 fulfilled，user 仍为 null，
   * gate 会误判无权把人踢走。
   */
    ...
    initializeAuth: create.asyncThunk(
      ...
      async (_, thunkAPI) => {
        ...
        if (tokenEntries.length !== (tokens?.length ?? 0)) {
          ...
          // Sequential: tokenManager.removeToken is read-modify-write on a shared
          // list (web localStorage / RN Keychain). Parallel removes race and can
          // leave invalid tokens behind.
      ...
      },
      {
        ...
        rejected: (state) => {
          // 初始化失败也算"已尝试过"——让 gate 正常判无权并 Navigate 踢走，
          // 而不是永远卡在 PageLoading。
    ...
    changeUser: create.asyncThunk(
      ...
      async (user: User, thunkAPI) => {
        ...
        try {
          ...
          // 加载新用户的 space 成员资格（不再自动进入 sticky 默认空间）
          await (
            dispatch as any
      ...
      {
        pending: (state) => {
          // 切账号期间旧管理员的管理页会继续可见可操作，造成视觉越权残留。
          // 立即进入 Loading 态并清掉 currentUser，直到新账号就绪。
        ...
        rejected: (state) => {
          // 切换失败也算"已尝试过"——设回 true 让 gate 正常判权（currentUser
          // 已被 pending 清成 null，check(null) 返回 false，自然 Navigate 踢走），
          // 否则 isInitialized 永久 false 会卡死在 PageLoading。
          state.isInitialized = true;
          state.isLoggedIn = false;
          state.currentToken = null;
        },
...
export const selectIsLoggedIn = (state: { auth: AuthState }) => state.auth?.isLoggedIn ?? false;
export const selectCurrentToken = (state: { auth: AuthState }) => state.auth?.currentToken ?? null;
