...
export const resolveMessageOwner = (input: {
  ...
  const dialogConfigUserId =
    typeof input.dialogConfigUserId === "string" &&
    input.dialogConfigUserId.trim().length > 0
      ? input.dialogConfigUserId.trim()
      : null;
  const currentAccountUserId =
    typeof input.currentAccountUserId === "string" &&
    input.currentAccountUserId.trim().length > 0
      ? input.currentAccountUserId.trim()
      : null;
