...
// Mention 节点类型
export interface SlateMentionNode {
  type: "mention";
...
export type SlateChild = SlateTextNode | SlateElementNode | SlateMentionNode;
    ...
    for (const child of mdastChildren) {
      ...
      switch (child.type) {
        ...
        case "text": {
          ...
          // 识别 @[type:id|label] 格式的 mention 语法
          ...
          while ((match = mentionRegex.exec(text)) !== null) {
            ...
            result.push({
              type: "mention",
              resourceType: match[1] as "tool" | "page" | "agent" | "space",
              resourceId: match[2],
              label: match[3],
              children: [{ text: "" }],
            });
