...
const dedupeSpaceContents = (
  ...
  const sorted = [...entries].sort((a, b) => {
    ...
    const aUpdated = toTimestampMs(aItem?.updatedAt);
    const bUpdated = toTimestampMs(bItem?.updatedAt);
  ...
  fixSpace: create.asyncThunk(
    async (spaceId: string, thunkAPI: any) => {
      ...
      changes.updatedAt = Math.max(
        Date.now(),
        toTimestampMs(spaceData.updatedAt) + 1
      );
    ...
    },
    {
      fulfilled: (state: SpaceState, action: any) => {
        ...
        const currentUpdatedAt = toTimestampMs(state.currentSpace.updatedAt);
        const nextUpdatedAt = toTimestampMs(nextSpace.updatedAt);
